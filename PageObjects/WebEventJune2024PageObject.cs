...
{
    public class WebEventJune2024PageObject(IWebDriver driver, SynchronizationContext? uiContext, AccessingElement uiElement) : BasePageObject(driver)
    {

        private static readonly string eventURL = @"https://tanki.su/ru/bagration-event/";
        ...
        private void SetHint(string hint)
        {
            UiContext?.Send(
                new SendOrPostCallback((s) =>
                {
                    uiElement.ToolTip = "Событие 'Операция Багратион': " + hint;
                }),
                null
                );
        }
